...
const AdminNotificationDetail = () => {
  ...
  const handleReadNotifications = (notifications) => {
    readNotification({
    ...
    if (isPeminjaman) {
      handleChangeAdminPageId(11);
    } else {
      handleChangeAdminPageId(12);
    }
  };
      ...
      <table className="mb-3 w-full">
        ...
        <tbody>
          {notifikasiData?.data?.length == 0 ? (
            ...
            notifikasiData?.data?.map((notifications) => {
              const { id, pesan, dibaca } = notifications;
                ...
                <tr key={id}>
                  <td className="border-b border-zinc-300 p-2">
                    <div className="w-full flex justify-start items-center">
                      ...
                      />
                      <button onClick={() => handleReadNotifications(notifications)} className="text-start">
