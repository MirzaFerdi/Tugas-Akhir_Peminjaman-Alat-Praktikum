...
const Kelas2TableData = () => {
  ...
  const { data: dataMahasiswaKelas2OnSearch } = useFetchOnMount({
    url: keywords === "" ? `/user/kelas/2` : `/user/search/mahasiswa/2/${keywords}`,
    method: "GET",
  });
  ...
  return (
    <React.Fragment>
      <div className="flex justify-between items-center px-4 lg:px-8 py-2 lg:py-4 bg-main shadow-sm shadow-main mx-6 lg:mx-8 rounded-xl text-white -mb-5 lg:-mb-10 relative">
        <h1 className="text-sm lg:text-2xl tracking-wide font-bold leading-none mr-4">Mahasiswa Kelas 2</h1>
      ...
      <div className="pb-5 pt-10 lg:pt-16 bg-white px-8 rounded-md shadow-md">
        <div className="grid grid-cols-1 lg:grid-cols-2 items-start mb-8">
          <div className="flex flex-col lg:items-start mb-3 lg:mb-0">
            <p className="text-md tracking-wide text-center lg:text-start">Tabel Data Mahasiswa Kelas 2</p>
        ...
        {isMobile ? (
          <div>
            {dataMahasiswaKelas2OnSearch?.data?.length > 0 ? (
              dataMahasiswaKelas2OnSearch?.data?.map((payloads) => {
              ...
              <div>
                <p className="p-2 text-xs text-center">
                  {dataMahasiswaKelas2OnSearch?.total === 0 && "Mahasiswa kelas 2 tidak ada!"}
                </p>
              </div>
            )}
          </div>
          ...
          <table className="mb-3 w-full">
            ...
            <tbody>
              {dataMahasiswaKelas2OnSearch?.data?.length > 0 ? (
                dataMahasiswaKelas2OnSearch?.data?.map((values) => {
                ...
                <tr>
                  <td colSpan={4} className="p-2 border text-xs text-center">
                    {dataMahasiswaKelas2OnSearch?.total === 0
                      ? "Mahasiswa kelas 2 tidak ada!"
